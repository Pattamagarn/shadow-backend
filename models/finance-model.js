...
module.exports.financeUpdate = (request, response) => {
    ...
    connection.query('UPDATE finance SET cash_amount = ?, aysel_amount = ?, update_at = ? WHERE email = ?', [requestCashAmount, requestAyselAmount, new Date(), requestEmail], (error, result) => {
        if(error){
